export const products = [
  {
    id: 1,
    name: 'Cолдатики',
    description: 'лучшие солдатики в мире',
    price: 10000,
    flagNew: true,
    flagSoon: false,

    imgs: [
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
    ],
  },
  {
    id: 2,
    name: 'Новые cолдатики 2',
    description: 'лучшие солдатики в мире 2',
    price: 5000,
    flagNew: false,
    flagSoon: false,

    imgs: [
      'https://i.pinimg.com/736x/79/81/0c/79810c5a590581159c637c8c574aab73.jpg',
      'https://i.pinimg.com/736x/79/81/0c/79810c5a590581159c637c8c574aab73.jpg',
      'https://i.pinimg.com/736x/79/81/0c/79810c5a590581159c637c8c574aab73.jpg',
    ],
  },
  {
    id: 3,
    name: '3 версия - солдатики 3 ',
    description: 'худшие солдатики в мире',
    price: 310000,
    flagNew: false,
    flagSoon: true,

    imgs: [
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
    ],
  },
  {
    id: 4,
    name: 'Пробую длинное название солдатики 4',
    description: 'После победы над Балрогом Гэндальф вернулся на Средиземье. Гэндальф теперь более могущественный Волшебник, чем когда-либо. Во время битвы за Минас Тирит именно Гэндальф подстрекает армию Гондора, вдохновляя их удерживать орков-захватчиков.\n' +
      '                Набор содержит компоненты, необходимые для сборки Gandalf White и Peregrin Took в 1 из 2 вариантах.\n' +
      '                1 вариант: Верхом на Shadowfax. При сборке вы можете не добавлять Peregrin Took, оставив только Гэндальфа на Shadowfax.\n' +
      '                2 вариант: Гэндальф представлен с Гламдрингом и посохом, а Перегрин Тук одет в униформу защитника Цитадели Минаса Тирита со шлемом в левой руке.\n' +
      '                Важно! Собрать миниатюры можно только в 1 варианте!\n' +
      '                Комплектация:\n' +
      '                14 деталей для сборки 2 миниатюр\n' +
      '                2 подставки 25mm\n' +
      '                1 подставка 40mm',
    price: 500,
    flagNew: false,
    flagSoon: false,

    imgs: [
      'https://i.pinimg.com/736x/9e/b7/eb/9eb7eb5180518f0949fbc050cd7af224.jpg',
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
    ],
  },{
    id: 5,
    name: 'Cолдатики',
    description: 'лучшие солдатики в мире',
    price: 10000,
    flagNew: true,
    flagSoon: false,

    imgs: [
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
    ],
  },
  {
    id: 6,
    name: 'Новые cолдатики 2',
    description: 'лучшие солдатики в мире 2',
    price: 5000,
    flagNew: false,
    flagSoon: false,

    imgs: [
      'https://i.pinimg.com/736x/79/81/0c/79810c5a590581159c637c8c574aab73.jpg',
      'https://i.pinimg.com/736x/79/81/0c/79810c5a590581159c637c8c574aab73.jpg',
      'https://i.pinimg.com/736x/79/81/0c/79810c5a590581159c637c8c574aab73.jpg',
    ],
  },
  /*{
    id: 7,
    name: '3 версия - солдатики ',
    description: 'худшие солдатики в мире',
    price: 310000,
    flagNew: false,
    flagSoon: true,

    imgs: [
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
    ],
  },
  {
    id: 8,
    name: 'Пробую длинное название солдатики 4',
    description: 'После победы над Балрогом Гэндальф вернулся на Средиземье. Гэндальф теперь более могущественный Волшебник, чем когда-либо. Во время битвы за Минас Тирит именно Гэндальф подстрекает армию Гондора, вдохновляя их удерживать орков-захватчиков.\n' +
      '                Набор содержит компоненты, необходимые для сборки Gandalf White и Peregrin Took в 1 из 2 вариантах.\n' +
      '                1 вариант: Верхом на Shadowfax. При сборке вы можете не добавлять Peregrin Took, оставив только Гэндальфа на Shadowfax.\n' +
      '                2 вариант: Гэндальф представлен с Гламдрингом и посохом, а Перегрин Тук одет в униформу защитника Цитадели Минаса Тирита со шлемом в левой руке.\n' +
      '                Важно! Собрать миниатюры можно только в 1 варианте!\n' +
      '                Комплектация:\n' +
      '                14 деталей для сборки 2 миниатюр\n' +
      '                2 подставки 25mm\n' +
      '                1 подставка 40mm',
    price: 500,
    flagNew: false,
    flagSoon: false,

    imgs: [
      'https://i.pinimg.com/736x/9e/b7/eb/9eb7eb5180518f0949fbc050cd7af224.jpg',
      'https://i.pinimg.com/originals/57/b7/aa/57b7aadf862a82a515cd0b03c618f8a1.jpg',
      'https://cdn.shopify.com/s/files/1/0657/9717/products/viking-warband_d1fdde27-414b-4687-9a2a-f0338b4d5fcb.jpg?v=1571438779',
    ],
  },*/
];
